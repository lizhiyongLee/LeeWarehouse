 ...
 */
@Data
@TableName("mes_qc_task_relate_total")
@EqualsAndHashCode(callSuper = false)
@Accessors(chain = true)
@ApiModel(value = "mes_qc_task_relate_total对象", description = "质检任务关联物料整体")
public class QcTaskRelateTotal extends ILSEntity {
     ...
     */
    @Excel(name = "对于在仓库中的才需要位置；原材料发到线边仓，也有个线边仓的位置。在制品不用位置表示，填入process。出库未入库的无位置状态1、有仓位编码的写入编码id;2、出库无位置的写入null;3、在制品写入process,最后一道工序也写入null标记生产完，与出库类似；", width = 15, dictTable = "mes_ware_storage", dicCode = "id", dicText = "storage_name")
    @ApiModelProperty(value = "对于在仓库中的才需要位置；原材料发到线边仓，也有个线边仓的位置。在制品不用位置表示，填入process。出库未入库的无位置状态 1、有仓位编码的写入编码id;2、出库无位置的写入null;3、在制品写入process,最后一道工序也写入null标记生产完，与出库类似；", position = 11)
    @TableField("storage_id")
    @Dict(dictTable = "mes_ware_storage", dicCode = "id", dicText = "storage_name")
