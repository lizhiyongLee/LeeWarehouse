...
@TableName("mes_work_order_line")
@ApiModel(value = "mes_work_order_line对象", description = "工单工序明细")
public class WorkOrderLine extends ILSEntity implements Comparable<WorkOrderLine> {
     ...
     * 1，分钟；2，小时；3，天
     */
    @Excel(name = "1，分钟；2，小时；3，天", width = 15)
    @TableField("setup_time_unit")
    @ApiModelProperty(value = "1，分钟；2，小时；3，天", position = 18)
    ...
    @ApiModelProperty(value = "加工时长", position = 19)
    ...
    @ApiModelProperty(value = "设备台时", position = 20)
    ...
    @Override
    public int compareTo(WorkOrderLine workOrderLine) {
        return this.getSeq() - workOrderLine.getSeq();
    }
}
